...
init -4 python:
    ...
    class ManifestState(object):
        ...
        def reset_all(self):
            ...
            self.role_plan_seed = random.SystemRandom().randint(100000, 999999999)
            self.role_windows = {}
            self.role_schedule = self.build_role_schedule()
        ...
        def weighted_role_choice(self, rng, code_weights):
            total_weight = float(sum(code_weights.values()))
        ...
        def late_role_weights(self, cid, week):
            return {
                "lena": {
                    "intern": 3,
                    "saboteur": 2,
        ...
        def build_role_schedule(self):
            rng = random.Random(self.role_plan_seed)
            self.role_windows = {
                "lena_bridge_week": rng.choice([3, 4, 5]),
                "sarah_support_week": rng.choice([2, 3, 4, 5, 6]),
            }
            schedule = {}
            ...
            for week in CORE_WEEKS:
                ...
                for cid, cdata in CHARACTER_POOL.items():
                    if week == 1:
                        week_schedule[cid] = cdata["variants"][0]["code"]
                        continue

                    code_weights = self.late_role_weights(cid, week)

                    if cid == "lena" and week in [3, 4, 5]:
                        if week == self.role_windows["lena_bridge_week"]:
                            code_weights = {"engineer": 2, "repair_key": 1}
                        else:
                            code_weights = {"intern": 2, "saboteur": 1}
                    elif cid == "sarah":
                        if week == self.role_windows["sarah_support_week"]:
                            code_weights = {"investigator": 2, "ally": 1}
                        else:
                            code_weights = {"panic": 2, "spy": 1}

                    week_schedule[cid] = self.weighted_role_choice(rng, code_weights)

                schedule[week] = week_schedule

            return schedule

        def get_scheduled_variant(self, cid):
            scheduled_roles = getattr(self, "role_schedule", {}) or {}
            week_schedule = scheduled_roles.get(self.week_index, {})
            code = week_schedule.get(cid)
            if not code:
                return None

            for variant in CHARACTER_POOL.get(cid, {}).get("variants", []):
        ...
        def roll_new_week(self, forced_seed=None):
            ...
            if not getattr(self, "role_plan_seed", None):
                self.role_plan_seed = random.SystemRandom().randint(100000, 999999999)
            if not getattr(self, "role_schedule", None):
                self.role_schedule = self.build_role_schedule()

            rng = random.Random(forced_seed if forced_seed is not None else self.loop_count)
            current_manifest = {}
            for cid, cdata in CHARACTER_POOL.items():
                variant = self.get_scheduled_variant(cid)
                if variant is None:
        ...
        def needs_weekly_proof(self, cid):
            target_cid = self.weekly_proof_target()
            return bool(target_cid and cid == target_cid and cid not in self.proofed_this_week)

        def mark_interaction_denial(self, cid, denied):
            if cid:
                if denied:
                    self.interaction_denials[cid] = True
                else:
                    self.interaction_denials.pop(cid, None)
                self.sync_progress()
        ...
        def get_role_code(self, cid):
            return self.manifest.get(cid, {}).get("variant_code")

        def partner_role_requirements(self):
            return {
                "lena": ["engineer", "repair_key"],
                "marcus": ["guardian", "paranoid"],
                "kenji": ["surgeon", "anomaly_scientist", "believer"],
                "sarah": ["investigator", "ally"],
                "thomas": ["veteran", "savior"],
                "hayes": ["leader"],
                "mira": ["quiet_researcher", "mathematician", "alien"],
            }

        def partner_sabotage_roles(self):
            return {
                "lena": ["saboteur"],
                "marcus": ["smuggler"],
                "sarah": ["spy"],
                "hayes": ["dictator", "secret_keeper", "suspected_saboteur"],
                "mira": ["fanatic"],
            }
        ...
        def resolve_partner_quest(self, quest_code, cid, stat=None, difficulty=55, reward=None, required_codes=None, sabotage_codes=None):
            role_code = self.get_role_code(cid)
            if sabotage_codes is None:
                sabotage_codes = self.partner_sabotage_roles().get(cid, [])
            if required_codes is None:
                required_codes = self.partner_role_requirements().get(cid)

            if role_code in sabotage_codes or not self.can_offer_help(cid):
                return "sabotage"

            if required_codes and role_code not in required_codes:
                return "failure"
        ...
        def quest_requirements(self):
            return {
                ...
                "quest_18_decoy_maneuver": {"authority": 54, "analysis": 55},
                "quest_19_nav_reflash": {"analysis": 72, "resilience": 60},
                "quest_20_civilian_evac": {"authority": 56, "empathy": 52},
            }
        ...
        def can_interact_with(self, cid):
            return cid not in self.blocked_characters and not self.interaction_denials.get(cid, False)
        ...
        def evaluate_ending(self):
            ...
            if self.stats["resilience"] < 45 and self.memory_stability < 52:
