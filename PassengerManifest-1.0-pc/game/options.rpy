...
define config.name = _("Passenger Manifest")
...
define build.name = "PassengerManifest"
...
define config.save_directory = "PassengerManifest-1773482796"

init -1 python:
    import os

    # Source runs and packaged builds should not share the same persistent profile.
    is_packaged_build = (
        os.path.isdir(os.path.join(config.basedir, "renpy"))
        and os.path.isdir(os.path.join(config.basedir, "lib"))
    )

    if not is_packaged_build:
        config.save_directory = "PassengerManifest-dev-1773482796"
...
init python:
    ...
    build.classify('**/navigation.json', None)
    build.classify('**/persistent', None)
    build.classify('.idea/**', None)
