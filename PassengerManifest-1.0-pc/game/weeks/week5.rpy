...
label w5_between_4_5_scene:
    scene bg technical
    na "Из глубины техотсека доносится короткий металлический удар."
    $ pm_story_impact("metallic_impact", pause=0.16, stop_after=True, duck_music=False)
    na "Несколько человек одновременно поднимают головы."
        ...
        "Сара помогает собрать хронологию":
            ...
            if manifest_state.can_interact_with("sarah"):
                if manifest_state.role_requirement_met("sarah", ["investigator", "ally"]):
                    ...
                    sc "Я не соберу здесь надёжную хронологию. Всё разъезжается раньше, чем складывается в доказательство."
                    ev "Значит, эта ветка тоже не даёт нам человека, который удержит общий рассказ."
            else:
