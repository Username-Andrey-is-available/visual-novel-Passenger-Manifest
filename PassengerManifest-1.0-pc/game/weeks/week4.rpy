...
label w4_between_5_6_scene:
    scene bg corridor
    na "Световая дорожка аварийного выхода на секунду вспыхивает ярче обычного."
    $ pm_story_impact("light_flicker", pause=0.12, stop_after=True, duck_music=False)
    na "И снова гаснет до штатного режима."
        ...
        "Секретная встреча с Сарой":
            ...
            if manifest_state.can_interact_with("sarah"):
                if manifest_state.role_requirement_met("sarah", ["investigator", "ally"]):
                    ...
                    sc "Сейчас я не удержу такую встречу в рабочем русле."
                    ev "Тогда и этот канал доверия в петле не откроется."
            else:
