...
label generic_role_quest(quest_code, partner_cid, stat_name, difficulty, reward_code, success_text, failure_text, sabotage_text):
    ...
    if not manifest_state.can_interact_with(partner_cid):
        ...
        return
    if not manifest_state.can_offer_help(partner_cid):
        $ manifest_state.take_action(quest_code)
        $ renpy.say(narrator, sabotage_text)
        return
    $ manifest_state.take_action(quest_code)
    if manifest_state.resolve_partner_quest(quest_code, partner_cid, stat_name, difficulty, reward_code):
...
label investigate_pilot_cabin_with_lena(action_code):
    ...
    $ pm_story_sfx("dead_pilots_reveal")
    na "Дверь пилотской кабины отходит в сторону. Внутри — два тела: Алик Хартманн и Нельсон Стюарт. Они мертвы уже давно."
    ...
    if manifest_state.role_requirement_met("hayes", ["secret_keeper", "suspected_saboteur"]):
        ...
        $ pm_story_sfx("captain_gunshots")
        na "Выстрел звучит коротко."
